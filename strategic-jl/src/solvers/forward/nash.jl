...
function solve(world::StrategicWorld, ::NashEquilibrium)::MixedNashResult
    ...
    # Parentheses matter: without them `&&` binds tighter than `||`, so the
    # error only fires when p1 has 2 acts and p2 doesn't.
    ...
    A = get(pf(a1, c1), p1, 0.0)
    E = get(pf(a1, c1), p2, 0.0)
    B = get(pf(a1, c2), p1, 0.0)
    F = get(pf(a1, c2), p2, 0.0)
    C = get(pf(a2, c1), p1, 0.0)
    G = get(pf(a2, c1), p2, 0.0)
    D = get(pf(a2, c2), p1, 0.0)
    H = get(pf(a2, c2), p2, 0.0)
    ...
    push!(prov,
        ProvenanceNode("pure_nash_search", "Chapter 7",
    ...
    if abs(denom_p) > 1e-10 && abs(denom_q) > 1e-10
        ...
        if 0 <= p <= 1 && 0 <= q <= 1
            ...
            push!(prov,
                ProvenanceNode("mixed_nash_equilibrium", "Chapter 7",
    ...
    if mixed === nothing && isempty(pure)
        push!(prov,
            ProvenanceNode("no_nash", "Chapter 7",
