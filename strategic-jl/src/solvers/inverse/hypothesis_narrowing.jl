...
function NarrowingSession(hypotheses::Vector{<:StrategicWorld})
...
function prune!(s::NarrowingSession, hypothesis_id::String, reason::String)
    ...
    s.hypotheses[idx] = HypothesisWorld(
        h.id, h.world, h.prior, 0.0, vcat(h.provenance, [node]))
...
function rule_in!(s::NarrowingSession, hypothesis_id::String, reason::String)
    ...
    boosted = HypothesisWorld(
        h.id, h.world, h.prior, h.posterior * 2.0, vcat(h.provenance, [node]))
    s.hypotheses[idx] = boosted
    # Renormalise
    total = sum(h.posterior for h in s.hypotheses)
    if total > 0
        s.hypotheses = [HypothesisWorld(
                            h.id, h.world, h.prior, h.posterior / total, h.provenance)
