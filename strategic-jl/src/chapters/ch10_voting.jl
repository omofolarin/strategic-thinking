...
function solve(world::StrategicWorld, ::VotingSolver)::VotingResult
    ...
    isempty(preferences) && return VotingResult(nothing,
        false,
        String[],
        Dict(),
        [
    ...
    push!(prov,
        ProvenanceNode("pairwise_comparison", "Chapter 10",
    ...
    for a in options
        if all(b -> b == a || get(get(pairwise, a, Dict()), b, false), options)
            winner = a
            push!(prov,
                ProvenanceNode("condorcet_winner", "Chapter 10",
    ...
    if winner === nothing
        ...
        push!(prov,
            ProvenanceNode("condorcet_cycle", "Chapter 10",
...
function _extract_preferences(world::StrategicWorld, trait_idx)::Dict{
